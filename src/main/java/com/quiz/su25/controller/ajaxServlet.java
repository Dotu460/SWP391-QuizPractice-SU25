...
import com.quiz.su25.config.VNPAYConfig;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (amountParam == null || amountParam.isEmpty()) {
                // Redirect back to price package menu with error message
                req.getSession().setAttribute("message", "Payment failed: Missing amount parameter");
                req.getSession().setAttribute("messageType", "error");
                resp.sendRedirect(req.getContextPath() + "/price-package-menu");
                return;
            }
            
            // Lưu thông tin package vào session để VNPAYController sử dụng
            if (packageIdParam != null && !packageIdParam.isEmpty()) {
                req.getSession().setAttribute("purchasePackageId", Integer.parseInt(packageIdParam));
                req.getSession().setAttribute("purchaseAmount", new java.math.BigDecimal(amountParam));
                System.out.println("Saved to session - Package ID: " + packageIdParam + ", Amount: " + amountParam);
            }
