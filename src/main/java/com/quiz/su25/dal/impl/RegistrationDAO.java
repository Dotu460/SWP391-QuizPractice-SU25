...
public class RegistrationDAO extends DBContext implements I_DAO<Registration> {
    ...
    public int countAll(int userId) {
        String sql = "SELECT COUNT(*) as total FROM registrations WHERE user_id = ?";
        try {
            connection = getConnection();
            statement = connection.prepareStatement(sql);
    ...
    public List<Registration> findAllPaginated(int offset, int limit) {
        String sql = "SELECT * FROM registrations ORDER BY registration_time DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        List<Registration> listRegistration = new ArrayList<>();
        try {
            connection = getConnection();
            statement = connection.prepareStatement(sql);
    ...
    public List<Registration> findAllPaginated(int userId, int offset, int limit) {
        String sql = "SELECT * FROM registrations WHERE user_id = ? ORDER BY registration_time DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        List<Registration> listRegistration = new ArrayList<>();
        try {
            connection = getConnection();
            statement = connection.prepareStatement(sql);
